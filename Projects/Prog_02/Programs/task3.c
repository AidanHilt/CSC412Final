...
int main(int argc, char* argv[]){
    ...
    */
    ...
    struct typeRangeAggregate aggregate = {.typeCount = 2};
    aggregate.types = calloc(2, sizeof(struct typeRange));

    aggregate.types[0].name = "Letters";
    aggregate.types[0].lowerBounds = "Aa";
    aggregate.types[0].upperBounds = "Zz";
    aggregate.types[0].length = 2;

    aggregate.types[1].name = "Numbers";
    aggregate.types[1].lowerBounds = "0";
    aggregate.types[1].upperBounds = "9";
    aggregate.types[1].length = 1;

    //So, the loop is mostly the same as task2
    while(exit == 0){
