...
//Helper function that loops through a C-string to find it's length, and then returns it.
//Stops at zero.
...
//Helper function that gets the length of a C-string with the strLen() function,
//And then loops through the string again, finding the last occurence of '/' or 
//'\' (just in case). Then, it dynamically constructs an array meant to cover
//all the characters from that last occurence, and then fills it with the remaining
//characters in the directory argument. It then returns the pointer to that array,
//expecting the caller to handle freeing the memory when it has used the string.
...
void calculatePrimesBetween(int m, int n){
    ...
    while(primeIterator <= limit){
        if(isPrime(primeIterator) == 1){
            ...
            primeIterator++;
            if(position == size - 1){
                primes = realloc(primes, size*2 * sizeof(int));
                size *= 2;
            }
        }else{
            primeIterator++;
...
//is greater than 3, we report to the user that there were too MANY arguments, and quit.
int main(int argc, char* argv[]){
    if(argc == 1){
        ...
        //The fact that sscanf is attempting to make 2 assignments is to handle the possible 
        //edge case where someone gives an argument like 12a. If it only succesfully makes one assignment, then
        //we know it only found a number. If it does not make it, then it either found no valid number, or 
        //a number and a string. 
        int n;
        char c;
        if(sscanf(argv[1], "%d%s", &n, &c) == 1 && n >= 1){
            calculateNPrimes(n);
            return 0;
        }else{
            char* exec = getExecName(argv[0]);
        ...
        int m, n;
        char c;
        if(sscanf(argv[1], "%d%s", &m, &c) == 1 && m >= 1){
            if(sscanf(argv[2], "%d%s", &n, &c) == 1 && n >= 1){
